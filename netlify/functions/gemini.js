...
exports.handler = async function(event, context) {
   ...
   // ... código inicial (imports y configuración) ...

    try {
        // Agregamos contextPath a lo que recibimos
        const { action, topic, contextPath } = JSON.parse(event.body);

        if (action === 'expand') {
            const schema = { /* ... mantén el mismo schema que ya tienes ... */ };
            ...
            const response = await ai.models.generateContent({
                ...
                3. Si es un concepto independiente y universal (ej. "Perro"), ignora el contexto y expándelo de forma general.
                4. Genera de 3 a 5 subconceptos clave. No repitas términos que ya existan en la jerarquía.`,
                config: {
                    responseMimeType: 'application/json',
                    responseSchema: schema,
                    temperature: 0.2 // Más bajo para mayor precisión conceptual
                }
            });
